 ...
 *******************************************************************************
 *  VERION HISTORY:
 *      v1.1.0 [18.12.2007] - Second release
 *                            Better debugging ,Attachments ,Button generation
 *                            ,Robust email handling and easier sandbox testing. 
...
class paypal_class {
   ...
   function paypal_class() {
      ...
      $this->ipn_log_file = 'digisell_debug.log';
      $this->ipn_response = '';
   ...
   function validate_product()
	{
		...
		global $products,$currency,$paypal_email;
   		...
   		{
   			$this->debug_log('Invalid Reciver E-Mail : '.$this->ipn_data['receiver_email'],false);  
         	return false;
   		}
...
if ($p->validate_ipn()) {
		 ...
		 $p_email = $p->ipn_data['payer_email'];
		 
		 $chave = base64_encode(RC4Crypt::encrypt($p_email,$download));
		 
		 $chave_enc = rawurlencode($chave);
		 
	     // Personalizando o script ---------------------
	     while(@db_existe_chave($chave)){
	     	$download = $product_id.'|'.$time;
	     ...
	     @db_insert($chave, $p->ipn_data['payer_email'], $p->ipn_data['txn_id']);
		 // ----------------------------------------------
      	 
      	 $download_link = $script_location.'digite-sua-mensagem?id='. $chave_enc;
         ...
         if(@send_mail($p->ipn_data['payer_email'],$body,$subject,$download_email))
         {
